...
uint256 constant PORTAL_TMONS_NUM = 10;
...
struct TMon {
    uint16[EQUIPPED_WEARABLE_SLOTS] equippedWearables; //The currently equipped wearables of the TMon
    ...
    uint256 usedSkillPoints; //The number of skill points this TMon has already used
    uint256 interactionCount; //How many times the owner of this TMon has interacted with it.
    address collateralType;
    uint40 claimTime; //The block timestamp when this TMon was claimed
    uint40 lastTemporaryBoost;
    uint16 hauntId;
    address owner;
    uint8 status; // 0 == portal, 1 == VRF_PENDING, 2 == open portal, 3 == TMon
    uint40 lastInteracted; //The last time this TMon was interacted with
    bool locked;
    address escrow; //The escrow address this TMon manages.
}
...
struct ItemType {
    ...
    uint16 minLevel; //The minimum TMon level required to use this item. Default is 1.
...
struct ERC721Listing {
    ...
    uint256 category; // 0 is closed portal, 1 is vrf pending, 2 is open portal, 3 is TMon
    uint256 priceInWei;
    uint256 timeCreated;
...
struct AppStorage {
    mapping(address => TMonCollateralTypeInfo) collateralTypeInfo;
    ...
    mapping(string => bool) TMonNamesUsed;
    ...
    mapping(uint32 => uint32) TMonToListingId;
    ...
    mapping(bytes32 => mapping(uint32 => LendingListItem)) TMonLenderLendingListItem; // ("listed" or "agreed") => listingId => LendingListItem
    mapping(address => mapping(bytes32 => uint32)) TMonLenderLendingHead; // user address => ("listed" or "agreed") => listingId => LendingListItem
...
contract Modifiers {
    ...
    modifier onlyTMonOwner(uint256 _tokenId) {
        require(LibMeta.msgSender() == s.tmons[_tokenId].owner, "LibAppStorage: Only tmons owner can call this function");
        _;
    }
    modifier onlyUnlocked(uint256 _tokenId) {
        require(s.tmons[_tokenId].locked == false, "LibAppStorage: Only callable on unlocked tmons");
        _;
    }
