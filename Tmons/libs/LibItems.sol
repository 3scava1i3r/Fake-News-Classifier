...
import {LibAppStorage, AppStorage, ItemType, TMon, EQUIPPED_WEARABLE_SLOTS} from "./LibAppStorage.sol";
import {LibERC1155} from "../../shared/libs/LibERC1155.sol";
...
library LibItems {
    ...
    function removeFromParent(
        ...
        if (_fromContract == address(this) && bal == 1) {
            TMon storage TMon = s.tmons[_fromTokenId];
            if (
                TMon.equippedWearables[LibItems.WEARABLE_SLOT_HAND_LEFT] == _id &&
                TMon.equippedWearables[LibItems.WEARABLE_SLOT_HAND_RIGHT] == _id
            ) {
    ...
    function checkWearableIsEquipped(uint256 _fromTokenId, uint256 _id) internal view {
        ...
        for (uint256 i; i < EQUIPPED_WEARABLE_SLOTS; i++) {
            require(s.tmons[_fromTokenId].equippedWearables[i] != _id, "Items: Cannot transfer wearable that is equipped");
        }
    }
}